...
for j = 1:length(At)
 if At(j) == 10*1e-3
  h1 = plot(freq(j),cfreq_e(j),'kv','MarkerSize',14);
 elseif At(j) == 20*1e-3
  h2 = plot(freq(j),cfreq_e(j),'bo','MarkerSize',14);
 elseif At(j) == 40*1e-3
  h3 = plot(freq(j),cfreq_e(j),'r^','MarkerSize',14);
 end
end

legend([h1 h2 h3],'H = 20mm','H = 40mm','H = 80mm')
...
for j = 1:length(At)
 if ~isnan(cvel_e(j))
  if At(j) == 10*1e-3
   h1 = plot(freq(j),cvel_e(j),'kv','MarkerSize',14);
  elseif At(j) == 20*1e-3
   h2 = plot(freq(j),cvel_e(j),'bo','MarkerSize',14);
  elseif At(j) == 40*1e-3
   h3 = plot(freq(j),cvel_e(j),'r^','MarkerSize',14);
  end
 end
end
