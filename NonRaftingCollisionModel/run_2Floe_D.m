% function run_2Floe_D(runno,Cd,Fs,Fd,tw)
%
% LJ YIEW
% Created on  May 2015
...
% Runs simulation (fn_SS_ode_S) with drift force only (no scattering force)
%
% INPUTS:
...
%  C     = 0.50;      % damping constant (mooring)
 K     = 0;      % spring constant  (mooring)
 C     = 0;      % damping constant (mooring)
 ...
 sep   = 1000e-3;     % initial separation [m]
 if ~exist('Fs','var');
 Fs    = 0; end     % wave scattering force
 if ~exist('Fd','var');
 Fd    = 0; end     % drift force
   ...
   while cc > 0
    ...
    if do_trans == 1
     ...
     if t1c(end) < trans_e
      ...
      n = 1;
      [t1_t,X1_t] = ode45(@(t,X) ...
                 fn_SS_ode_S(t,X,0,WaveParam,FloeParam,Coeff,Mooring,Trans,n), ...
                 tspan_t,IC1);
      ...
      %   TRANSIENT SOLUTION
      Trans.t = 1;
      Coeff.tw = tw + (400e-3+sep)/(freq*lambda);
      n = 2;
%       Trans.f.c = f.c + k*dist; % ### NO DRIFT
      [t2_t,X2_t] = ode45(@(t,X) ... 
                 fn_SS_ode_S(t,X,dist,WaveParam,FloeParam,Coeff,Mooring,Trans,n), ...
                 tspan_t,IC2);
      %   STEADY STATE SOLUTION          
      IC2s    = X2_t(end,:);
      Trans.t = 0;
